...
class ProductTemplate(models.Model):
    ...
    def button_fix_available_zero(self):
        ...
        if move_lines:
            ...
            if len(move_ids) > 1:
                self.env.cr.execute(
                    """ UPDATE stock_move SET state = 'confirmed' WHERE id in %s ;""" % (tuple(move_ids), ))
            elif len(move_ids) == 1:
                self.env.cr.execute(
                    """ UPDATE stock_move SET state = 'confirmed' WHERE id = %s ;""" % (move_ids[0]))
